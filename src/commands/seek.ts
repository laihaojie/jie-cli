 ...
 * @param {string} [options.startPath] - 起始文件夹路径，默认为当前文件夹
 * @param {number} [options.size] - 文件大小阈值，默认为10
 * @param {string} [options.unit] - 单位（'kb', 'mb', 'gb'），默认为'mb'
...
export async function findLargeFiles(options: FindLargeFilesOptions = {}) {
  ...
  const maxShow = options.maxShow || 10
   ...
   */
  async function traverseDirectory(dir, depth = 0) {
    if (depth > MAX_DEPTH) {
      console.error(chalk.red(`目录深度超过 ${MAX_DEPTH}，跳过: ${dir}`))
      return
    }
    try {
      // 检查是否需要排除当前文件夹
  ...
  try {
    ...
    await traverseDirectory(absolutePath, 0)
  }
  catch (error: any) {
