...
> ⚠️ **安全提示**：默认不启用鉴权，同一网络下任何人都能在你电脑上执行命令。仅在可信内网使用！如需开启鉴权，设置环境变量 `JIE_AUTH_TOKEN`，此后 WebSocket 连接需带 `?token=<token>`。
>
> 局域网其他设备首次访问若连不上，通常是 Windows 防火墙拦截——放行 Node.js 监听 32677 端口即可。
